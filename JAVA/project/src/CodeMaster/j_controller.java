...
public class j_controller {
    ...
    private void loadQuestions() {
        ...
        try {
            ...
            ResultSet resultSet = statement.executeQuery("SELECT * FROM JAVA"); // Change table name as necessary
    ...
    private String getSolutionFromDatabase(int srNo) {
        ...
        try {
            Connection connection = DriverManager.getConnection(url, user, password);
            Statement statement = connection.createStatement();
            ResultSet resultSet = statement.executeQuery("SELECT solution FROM JAVA WHERE Sr_no = " + srNo);
            if (resultSet.next()) {
    ...
    private String getDescriptionFromDatabase(int srNo) {
        ...
        try {
            Connection connection = DriverManager.getConnection(url, user, password);
            Statement statement = connection.createStatement();
            ResultSet resultSet = statement.executeQuery("SELECT description FROM JAVA WHERE Sr_no = " + srNo); // Change
                                                                                                                // to
                                                                                                                // your
                                                                                                                // column
                                                                                                                // name
