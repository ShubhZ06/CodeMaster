...
public class cpp_controller {
    ...
    private void loadQuestions() {
        ...
        try {
            ...
            ResultSet resultSet = statement.executeQuery("SELECT * FROM CPP"); // Change table name as necessary
    ...
    private String getSolutionFromDatabase(int srNo) {
        ...
        try {
            Connection connection = DriverManager.getConnection(url, user, password);
            Statement statement = connection.createStatement();
            ResultSet resultSet = statement.executeQuery("SELECT solution FROM CPP WHERE Sr_no = " + srNo);
            if (resultSet.next()) {
    ...
    private String getDescriptionFromDatabase(int srNo) {
        ...
        try {
            Connection connection = DriverManager.getConnection(url, user, password);
            Statement statement = connection.createStatement();
            ResultSet resultSet = statement.executeQuery("SELECT description FROM CPP WHERE Sr_no = " + srNo); // Change
                                                                                                               // to
                                                                                                               // your
                                                                                                               // column
                                                                                                               // name
