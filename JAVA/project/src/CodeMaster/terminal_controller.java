...
public class terminal_controller {
    @FXML
    private Text question_holder;
    @FXML
    private Text description_holder;
    @FXML
    ...
    // Method to set the question and description
    public void setQuestionAndDescription(String question, String description) {
        question_holder.setText(question);
        description_holder.setText(description);
    }
    ...
    private Process compileOrRun(File codeFile, String language, File outputDir) throws IOException {
        switch (language) {
            case "C":
                return Runtime.getRuntime().exec(new String[]{"gcc", codeFile.getAbsolutePath(), "-o", new File(outputDir, "temp_code.exe").getAbsolutePath()});
            case "C++":
                return Runtime.getRuntime().exec(new String[]{"g++", codeFile.getAbsolutePath(), "-o", new File(outputDir, "temp_code.exe").getAbsolutePath()});
            case "Java":
    ...
    private void captureProgramOutput() {
        new Thread(() -> {
            try {
                ...
                while ((outputLine = outputReader.readLine()) != null || (outputLine = errorReader.readLine()) != null) {
                    if (outputLine != null) {
    ...
    public void initData(String question, String description) {
        setQuestionAndDescription(question, description); // Set question and description
    }
}
